...
class JutsuClassifier():
    def __init__(self,
        ...
        if not huggingface_hub.repo_exists(self.model_path):
            ...
            if data_path is None:
                raise ValueError('dara is required')
    ...
    def train_model(self , train_data,test_data,class_weights):
        ...
        # ✅ Explicitly save config with model_type
        from transformers import AutoConfig
        config = AutoConfig.from_pretrained(
            self.model_name,
            num_labels=self.num_labels,
            id2label=self.label_dict,
            label2id={v: k for k, v in self.label_dict.items()}
        )
    ...
    def load_data(self , data_path):
        ...
        data['jutsu']=data['jutsu_type_simplified']
        data=data[['text','jutsu']]
        data=data.dropna()
        ...
        tokenizer_train=train_dataset.map(lambda examples: self.preprocess_function(self.tokenizer , examples),
                                batched=True  )


        tokenizer_test=test_dataset.map(lambda examples: self.preprocess_function(self.tokenizer , examples),
                                        batched=True  ) 
